...
class Davis(VideoDataset):
  def __init__(self, root, mode='train', resize_mode=None, resize_shape=None, tw=8, max_temporal_gap=8, num_classes=2,
               imset=None):
    # print("---- PKB ----", root, mode, resize_mode, resize_shape, tw, max_temporal_gap, num_classes, imset) # Output: /globalwork/data/DAVIS-Unsupervised/DAVIS/ train resize_short_edge_and_crop (480, 854) 8 12 2 2016/val.txt
    self.imset = imset
    self.videos = [] # list of the 2017/val.txt file
    self.num_frames = {} # key is the video name and value is the number of frames
    self.num_objects = {} # key is the video name and value is the number of semantic objects in the 0th frame mask (excluding background)
    self.shape = {} # key is the video name and value is the shape of the corresponding 0th frame of video
    self.raw_samples = []
  ...
  def filter_samples(self, video):
    '''
      Chooses a video sample from a list of self.raw_samples
    '''
  ...
  def create_sample_list(self):
    ...
    with open(os.path.join(self.root, "ImageSets",_imset_f), "r") as lines:
      for line in lines:
        _video = line.rstrip('\n')
        self.videos += [_video]
        img_list = list(glob.glob(os.path.join(image_dir, _video, '*.jpg'))) # stores all the images of a video
        img_list.sort()
        ...
        num_objects = np.max(_mask) # stores 1 for all the videos
