...
def resize_fixed_size(tensors, size):
  ...
  for key in tensors.keys():
    ...
    if len(tensor.shape) > 2:
      tensors_resized[key] = imresize(tensor, dsize=size[::-1], interpolation=cv2.INTER_LINEAR)
    else:
      tensors_resized[key] = imresize(tensor, dsize=size[::-1], interpolation=cv2.INTER_NEAREST)
