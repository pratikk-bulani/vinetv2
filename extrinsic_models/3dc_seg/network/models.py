...
class Encoder3d(nn.Module):
  def __init__(self, backbone, tw, pixel_mean, pixel_std):
    ...
    if backbone.PRETRAINED_WTS and False: # I have added the "and False"
...
class SaliencyNetwork(BaseNetwork):
  ...
  def forward(self, x, ref=None):
    # print(x.shape, x.dtype, x.min(), x.max()) # torch.Size([1, 3, 8, 480, 864]) torch.float32 tensor(0., device='cuda:0') tensor(1., device='cuda:0')
    r5, r4, r3, r2 = self.encoder.forward(x, ref)
    # print(r5.shape, r4.shape, r3.shape, r2.shape) # torch.Size([1, 2048, 4, 15, 27]) torch.Size([1, 1024, 8, 30, 54]) torch.Size([1, 512, 16, 60, 108]) torch.Size([1, 256, 32, 120, 216])
    # exit(0)
